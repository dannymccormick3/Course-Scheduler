...
def course_scheduler (course_descriptions, goal_conditions, initial_state):
	...
	schedule, tot_hours_per_semester = create_satisfying_schedule(course_descriptions, goal_conditions_map, initial_state_map, tot_hours_per_semester, 1)
	if len(schedule) == 0:
		return schedule
...
def create_satisfying_schedule (course_descriptions, goal_conditions, completed_courses, tot_hours_per_semester, depth):
	"""
	PRE:
		course_descriptions: Dictionary of courses with the following form.
			key - Course(program='<department>', designation='<number>') e.g. Course(program='CS', designation='2201')
	...
	if goal_class in completed_courses:
		if completed_courses[goal_class] <= semester_assignment:
			...
			return create_satisfying_schedule (course_descriptions, goal_conditions, completed_courses, tot_hours_per_semester, depth)
		else:
...
def assign_course_to_semester(course, goal_conditions, completed_courses, tot_hours_per_semester, course_info, semester, course_descriptions, depth):
	...
	if len(course_info[2]) == 0:
		return create_satisfying_schedule(course_descriptions, list(goal_conditions), dict(completed_courses), dict(tot_hours_per_semester), depth+1)
